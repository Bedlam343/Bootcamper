...
exports.getBootcamps = asyncHandler(async (req, res, next) => {
  // this method is using a middleware (advancedResults)
...
exports.bootcampPhotoUpload = asyncHandler(async (req, res, next) => {
  ...
  // Check file size
  if (file.size > keys.MAX_FILE_UPLOAD) {
    return next(
      new ErrorResponse(
        `Please upload an image less than ${keys.MAX_FILE_UPLOAD}`,
        400
      )
    );
  }

  // Create custome filename for photo
  // and the extension
  file.name = `photo_${bootcamp._id}${path.parse(file.name).ext}`;
  // console.log(file.name);

  // upload the photo
  file.mv(`${keys.FILE_UPLOAD_PATH}/${file.name}`, async (err) => {
    if (err) {
      console.error(err);
      return next(new ErrorResponse(`Problem with file upload`, 500));
    }

    // insert file.name into the DB
